...
constexpr auto K_SOURCE    = "stream/source";
...
void saveSettings(const Settings& s) {
    ...
    q.setValue(K_SOURCE,   int(s.source));
...
Settings loadSettings() {
    ...
    s.source = static_cast<VideoSource>(q.value(K_SOURCE, int(s.source)).toInt());
...
void SettingsDialog::buildStreamTab(QWidget* tab) {
    ...
    m_sourceCombo->addItem(tr("Захват экрана"), int(VideoSource::Screen));
    m_sourceCombo->addItem(tr("Тестовая картинка (testsrc2)"), int(VideoSource::TestPattern));
    ...
    form->addRow(tr("Источник"), m_sourceCombo);
}
...
void SettingsDialog::loadFromSettings() {
    ...
    m_sourceCombo->setCurrentIndex(m_sourceCombo->findData(int(m_settings.source)));
...
void SettingsDialog::writeBackFromUi() {
    ...
    m_settings.source = static_cast<VideoSource>(m_sourceCombo->currentData().toInt());
